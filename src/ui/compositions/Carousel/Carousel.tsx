...
interface SliderDotButtonProps extends React.ComponentPropsWithRef<'button'> {
  isActive?: boolean
  index: number
}
...
const CarouselPaginationItem = ({
  ...
  return (
    <button
      type="button"
      className={cn(
        'w-6 h-1.5 rounded-full cursor-pointer',
    ...
    >
      {children}
      <span className="sr-only">{`${isActive ? 'Active' : 'Inactive'} slide ${index + 1}`}</span>
    </button>
  )
}
